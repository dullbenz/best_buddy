...
are working directories, so copy the published subset here before building. The
...
|---|---|
| `holders.csv` | the exact list the bucket-2 Merkle root commits to, so anyone can rebuild the tree from it and check the root matches on-chain |
...
the holders it was trading against, and nothing could ever claim it, because a
PDA has no private key, so that share would sit unclaimed until the sweep
handed it to
